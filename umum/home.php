<!--content-->
<style>
#customers {
    font-family: "Trebuchet MS", Arial, Helvetica, sans-serif;
    border-collapse: collapse;
    width: 100%;
}

#konten, .thumbnail{
    border: 1px solid #ddd;
    padding: 8px;
    border-radius: 10px;
    background: white;
}

.thumbnail:hover {background-color: #ddd;}

#customers th {
    padding-top: 12px;
    padding-bottom: 12px;
    text-align: left;
    background-color: #4CAF50;
    color: white;
}
#konten{
  background-color: #4CAF50;
}

</style>

<div class="row" id="konten">
  <div class="col-sm-6 col-md-4">
    <div class="thumbnail">
      <img src="img/home/galeri.jpg" alt="" width="100%" height="355px">
      <div class="caption">
        <h3>Galeri</h3>
        <p>Potret daerah Indramayu & destinasi wisata nya.</p>
        <p><a href="index.php?page=galeri" class="btn btn-primary" role="button">Lihat</a> <a href="#" class="btn btn-default" role="button"></a></p>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-4">
    <div class="thumbnail">
      <img src="img/home/babad.jpg" width="100%" height="273px" alt="">
      <div class="caption">
        <h3>Babad Dermayu</h3>
        <p>Raden Wiralodra iku anak Bupati Bagelan, Raden Singalodra. Lagi nonomane Raden Singalodra seneng luruh elmu lan seneng tapa brata. Saben lelungan ditiburi pawongane, arane Ki Tinggil.</p>
        <p><a href="index.php?page=babad" class="btn btn-primary" role="button">Lihat</a> <a href="" class="btn btn-default" role="button"></a></p>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-4">
    <div class="thumbnail">
      <img src="img/home/news.jpg" width="100%" height="295px">
        <div class="caption">
          <h3>Berita Tentang Indramayu</h3>               
          <p>Klik Button di bawah ini untuk melihat berita sekitar Indramayu!</p>
          <p><a href="http://www.indramayupost.com" class="btn btn-primary" role="button">Lihat</a> <a href="#" class="btn btn-default" role="button"></a></p>
        </div>
    </div>
  </div>
</div>

<div class="row" id="konten">
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="img/home/pelajaran.jpg" alt="" width="100%" height="397px">
      <div class="caption">
        <h3>Pelajaran</h3>
        <p>Merupakan isi dari penerapan kehidupan sehari-hari masyarakat indramayu </p>
        <p><a href="index.php?page=pelajaran" class="btn btn-primary" role="button">Lihat</a> <a href="" class="btn btn-default" role="button"></a></p>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="img/home/aksara.png" alt="" width="100%" height="315px">
      <div class="caption">
        <h3>Aksara Jawa</h3>
        <p>Aksara Jawa, merupakan salah satu peninggalan budaya yang tak ternilai harganya. Bentuk aksara dan seni pembuatannya pun menjadi suatu peninggalan yang patut untuk dilestarikan.</p>
        <p><a href="index.php?page=aksara" class="btn btn-primary" role="button">Lihat</a> <a href="#" class="btn btn-default" role="button"></a></p>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="img/home/materi.jpg" alt="" width="100%" height="320px">
      <div class="caption">
        <h3>Materi Pelajaran</h3>
        <p>Materi :</p>
        <ul>
          <li>Mudah</li>
          <li>Sedang</li>
          <li>Sulit</li>
        </ul>
        <p>Di Setiap materi terdapat kuis </p>
        <p>
        <a href="index.php?page=mudah" class="btn btn-primary" role="button">Mudah</a>
        <a href="index.php?page=sedang" class="btn btn-primary" role="button">Sedang</a>
        <a href="index.php?page=sulit" class="btn btn-primary" role="button">Sulit</a>
        </p>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="img/home/apaitu.jpg" alt="" width="100%" height="310px">
      <div class="caption">
        <h3>Apa Itu Bahasa Indramayu</h3>
        <p>"Krama Language & Learning" adalah program belajar bahasa Indramayu yang diproduksi oleh Mahasiswa Polindra. Anda dapat belajar bersama kami secara gratis.</p>
        <p><a href="index.php?page=apaitu" class="btn btn-primary" role="button">Lihat</a> <a href="#" class="btn btn-default" role="button"></a></p>
      </div>
    </div>
  </div>
</div>
         <div class="row" id="konten">
          <div class="col-sm-6 col-md-3">
            <div class="thumbnail">
              <img src="img/home/manfaat.jpg" width="100%" height="100%">
              <div class="caption">
                <h3>Manfaat</h3>
                <ul>
                  <li>
                    Membiasakan Menggunakan Bahasa indramayu yang baik dan sopan
                  </li>
                  <li>
                    memperkenalkan bahasa indramayu ke masyarakat umum
                  </li>
                  <li>
                    tidak bingung ketika teman menggunakan bahasa indramayu
                  </li>
                </ul>            
              </div>
            </div>
          </div>
          <div class="col-sm-6 col-md-3">
            <div class="thumbnail">
              <img src="img/home/pertanyaan.jpg" width="100%" height="305px">
              <div class="caption">
                <h3>Daftar Pertanyaan </h3>
                <p>Lihat Daftar Pertanyaan dari pengunjung yang lain, klik tombol di bawah !!!</p>
                <p><a href="index.php?page=pertanyaan" class="btn btn-primary" role="button">Lihat</a> <a href="#" class="btn btn-default" role="button"></a></p>
              </div>
            </div>
          </div>
          <div class="col-sm-6 col-md-3">
            <div class="thumbnail">
              <img src="img/home/kontak.jpg" width="100%" height="339px">
              <div class="caption">
                <h3>Contact</h3>
                <p>Silahkan ajukan pertanyaan, kritik & saran anda untuk memperbaiki website kami</p>
                <p><a href="index.php?page=contact" class="btn btn-primary" role="button">Lihat</a> <a href="#" class="btn btn-default" role="button"></a></p>
              </div>
            </div>
          </div>
          <?php
            if($Id_Admin){

            }
            else if($Id_User){

            }
            else{
          ?>    
          <div class="col-sm-6 col-md-3">
            <div class="thumbnail">
              <img src="img/home/login.jpg" width="100%" height="338px">
              <div class="caption">
                <h3>Login</h3>
                <p>Silahkan buat akun anda yang berfungsi untuk mengakses situs kuis pada website kami</p>
                <p><a href="reg.php" class="btn btn-primary" role="button">Lihat</a> <a href="#" class="btn btn-default" role="button"></a></p>
              </div>
            </div>
          </div>
          <?php
            }
          ?>  
        </div>

         <div class="row" id="konten">
          
        </div>
<!-- content -->